...
boost::phoenix::function<fargs_impl> fargs;
    ...
    qi::grammar<std::string::const_iterator, std::string(), ascii::space_type>
{
        ...
        base_type(file_)
    {
        ...
        identifier = (lexeme[char_("a-zA-Z_") >> *(alnum | char_("_"))]) - keywords_table;
		...
		repeat_statement = no_case[lit("REPEAT")]
			   >> statement_sequence
			   >> no_case[lit("UNTIL")]
			   >> boolean_expression;

		open_while_statement = no_case[lit("WHILE")]
			   >> boolean_expression
			   >> no_case[lit("DO")]
			   >> open_statement;

		closed_while_statement = no_case[lit("WHILE")]
			   >> boolean_expression
			   >> no_case[lit("DO")]
			   >> closed_statement;
		...
		open_with_statement = no_case[lit("WITH")]
			   ...
			   >> no_case[lit("DO")]
			   >> open_statement; //TODO: unsupported error
		...
		variable_access = identifier [_val = _1]
		 | indexed_variable [_val = _1]
		 | field_designator [_val = _1]
		 | (variable_access >> lit("^")) [_val = "*" + _1];

		indexed_variable = variable_access [_val += _1]
				>> lit("[") [_val += "["]
		...
		direction = no_case[lit("TO")] | no_case[lit("DOWNTO")];
