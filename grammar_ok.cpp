    ...
    qi::grammar<std::string::const_iterator, std::string(), ascii::space_type>
{
        ...
        base_type(file_)
    {
        ...
        label_declaration_part = label_list [_val = ""]>> lit(';'); 

        constant_definition = identifier [_val = "#define " + _1] >> lit('=') [_val += " "] >>
            cexpression_ [_val += _1] >> lit(';'); 
		...
		structured_type_ = array_type.alias();
		...
		procedural_parameter_specification = procedure_heading.alias();
		functional_parameter_specification = function_heading.alias(); 

		procedure_identification = no_case[lit("PROCEDURE")] >> identifier;
		procedure_block = function_block.alias();
		...
		open_statement =  non_labeled_open_statement.alias();
		closed_statement =  non_labeled_closed_statement.alias(); 
